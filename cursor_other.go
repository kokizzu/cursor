...
func (c *Cursor) Up(n int) {
	if n > 0 {
		_, _ = fmt.Fprintf(c.writer, "\x1b[%dA", n)
	}
}
...
func (c *Cursor) Down(n int) {
	if n > 0 {
		_, _ = fmt.Fprintf(c.writer, "\x1b[%dB", n)
	}
}
...
func (c *Cursor) Right(n int) {
	if n > 0 {
		_, _ = fmt.Fprintf(c.writer, "\x1b[%dC", n)
	}
}
...
func (c *Cursor) Left(n int) {
	if n > 0 {
		_, _ = fmt.Fprintf(c.writer, "\x1b[%dD", n)
	}
}
...
func (c *Cursor) HorizontalAbsolute(n int) {
	...
	_, _ = fmt.Fprintf(c.writer, "\x1b[%dG", n)
}
...
func (c *Cursor) Show() {
	_, _ = fmt.Fprint(c.writer, "\x1b[?25h")
}
